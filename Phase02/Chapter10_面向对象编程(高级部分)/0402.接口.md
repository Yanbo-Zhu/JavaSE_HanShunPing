...
3. 一个类可以实现多个接口, ，逗号隔开
5. 一个普通类实现接口,就必须将该接口的所有方法都实现,可以使用alt+enter 来解决
6. 抽象类去实现接口时，可以不实现接口的抽象方法；
7.  一个接口不能继承其它的类，但是可以继承别的接口。interface A extens B,C{}
8. 继承的同时，再去实现接口  class A extends B inplements C{.....}
9. 接口的修饰符只能是public和默认，这点和类的修饰符是一样的。 public class Interface03 {}, 或者 class Interface03 {}
10. 4. 接口中所有的方法是public 方法, 接口中抽象方法，可以不用abstract 修饰
11. 接口中可以有变量（但是变量不能用private和protected修饰），只能是final的，而且是public static final 修饰符，比如：int a = 1；实际上是public static final int a = 1；（必须初始化） 
	1. 接口中的变量，本质上都是static的，而且是final，不管你加不加static修饰；
	2. 在java开发中，经常把常用的变量，定义在接口中，作为全局变量使用。
12. 接口中属性的访问形式：接口名.属性名；
...
多态就是在后期绑定这种机制上实现的。
————————————————
版权声明：本文为CSDN博主「PistonType」的原创文章，遵循CC 4.0 BY-SA版权协议，转载请附上原文出处链接及本声明。
原文链接：https://blog.csdn.net/u010505246/article/details/91860668
