...
3. 对于编译异常, 程序中必须写有有相关代码 (try-catch, 或者 throws) 进行处理, 不然会在 IDE 显示出错误
	1. ![](image/Pasted%20image%2020230409140225.png)
4. 对于运行时的异常, 程序中如果没有处理, 默认就是 throws的方式处理
5. 子类重写父类的方法的时候, 对于抛出异常的规定:
	1. 子类重写的方法, 所抛出的异常类型 要么 和父类抛出的异常一致, 要么为父类拍出的异常的类型的子类型 . 必须有父子之间的关系 
6. <mark>在throws 过程中,如果有方法 try-catch, 就相当于处理异常, 就可以不必throws </mark>
...
### 5.1.1 为什么 喜欢继承 运行异常, 不喜欢用 继承编译异常 : 
